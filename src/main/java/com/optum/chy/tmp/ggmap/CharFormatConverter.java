...
public class CharFormatConverter {  
	...
	static {  
		...
		ASCII2EBCDIC[0x5B] = (byte)0xBA;	// Defect fix 9/18/17 - replaced x'4A' ¢ with x'BA' [
		ASCII2EBCDIC[0x5C] = (byte)0xE0;
		ASCII2EBCDIC[0x5D] = (byte)0xBB;	// Defect fix 9/18/17 - replaced x'5A' ! with x'BB' ]
		...
		ASCII2EBCDIC[0xA1] = (byte)0x43;
		ASCII2EBCDIC[0xA2] = (byte)0x4A;
		ASCII2EBCDIC[0xA3] = (byte)0x44;
		ASCII2EBCDIC[0xA4] = (byte)0x45;
		ASCII2EBCDIC[0xA5] = (byte)0xB2;
		ASCII2EBCDIC[0xA6] = (byte)0x6A;
		...
		ASCII2EBCDIC[0xE2] = (byte)0x42;
		ASCII2EBCDIC[0xE3] = (byte)0x46;
	...
	private byte convertByteA2E(byte in) { return ASCII2EBCDIC[in & 0xFF]; }  
	private byte convertByteE2A(byte in) { return EBCDIC2ASCII[in & 0xFF]; }
	...
	public byte[] convertStrToEBCDIC(byte[] in) {  
		if (in!=null) {
			//Byte priorByte = (byte)0x20;;					// Initialize priorByte to a space
			//boolean openBracket = false;
			byte[] out = new byte[in.length];  
			for (int i = 0; i < in.length; ++i) {  
				out[i] = convertByteA2E(in[i]);
				if (out[i] == (byte)0xBA) out[i] = (byte)0xAD;	// Open Square bracket "["  (IBM-1047) 
				if (out[i] == (byte)0xBB) out[i] = (byte)0xBD;	// Close Square bracket "]" (IBM-1047) 
				/**
				// If "[" && ":" then replace x'BA' with x'AD'
				if (in[i] == (byte)0x5B && priorByte == (byte)0x3A) {	
					openBracket = true;
					out[i] = (byte)0xAD;	// Open Square bracket "["  (IBM-1047)
				}
				// If "]" && openBracket then replace x'BB' with x'BD'
				if (in[i] == (byte)0x5D && openBracket) {	
					out[i] = (byte)0xBD;	// Close Square bracket "]" (IBM-1047)
					openBracket = false;
				}
				if (in[i] != (byte)0x20) {
					priorByte = in[i];
	...
	/**  
	 * Convert an array of EBCDIC bytes to ASCII.  
	 *   
	 * @param in -- an array containing EBCDIC data.  
	 * @return -- an array containing ASCII data.  
	 */  

	public byte[] convertStrToASCII(byte[] in) {  
		if (in!=null) {  
			byte[] out = new byte[in.length];  
			for (int i = 0; i < in.length; ++i) {  
				out[i] = convertByteE2A(in[i]); 
				if (out[i] == (byte)0xD5) out[i] = (byte)0x5B;	// Open Square bracket  "[" (IBM-1047)
				if (out[i] == (byte)0xE5) out[i] = (byte)0x5D;	// Close Square bracket "]" (IBM-1047)
			}  
			return out;  
		}  
		return null;  
